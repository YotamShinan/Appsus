...
function getCurrnecyIcon(currency) {
...
function query(filterBy) {
    if (!gBooks) gBooks = storageService.load('books', gDefaultBooks)
    if (!filterBy) { return gBooks } else {
        var { bookName, maxPrice, minPrice } = filterBy
        maxPrice = maxPrice ? maxPrice : Infinity
        minPrice = minPrice ? minPrice : 0
        return gBooks.filter(book => book.title.includes(bookName) &&
